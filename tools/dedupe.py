...
PACKAGING_SUFFIXES = ("-bin", "-deb", "-app", "-appimage", "-static")
...
def write_sources(icons: dict[str, str]) -> None:
    header, _, _ = SOURCES.read_text().partition("[icons]")
...
def write_exceptions(aliases: dict[str, str], per_adapter: str) -> None:
    header, _, _ = EXCEPTIONS.read_text().partition("[default]")
...
def main() -> int:
    ...
    after = EXCEPTIONS.read_text().partition("[default]")[2]
