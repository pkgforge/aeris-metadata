...
entry at all: `-bin`, `-deb`, `-app`, `-appimage` and `-static` are dropped
when nothing matches, so `krita-bin` finds `krita` on its own. Saying it of a
name says it of the built forms too, so the line above also covers
`krita-devel-bin`.

Suffixes marking a different build, such as `-git`, `-esr` or `-nightly`, are
left alone: those are often a different program to look at, and Firefox draws a
different fox for each channel.
